...
__global__ void rmsnorm_twoPassAlgo_e8_prefetch(float4 *output, const float4 *input,
    ...
    const int overlap_boundary = (static_cast<int>(n_8 * prefetch_ratio) / bdimx) * bdimx;
    if constexpr (overlap_type == OverlapType::prefetch) {
        // compute overlap

        // load current block
        uint32_t weight_bytes =
            (uint32_t(n * prefetch_ratio) * sizeof(float) + 0xF) & ~0xF;  // zeroing last 4 bit
        if (tid == 0) {
    ...
    for (int index = tid; index < n_8; index += bdimx) {
        ...
        if constexpr (overlap_type != OverlapType::none)
            if (index >= overlap_boundary) asm volatile("griddepcontrol.launch_dependents;");
    }
}
...
// prefetch
#define INSTANTIATE_RMSNORM_TYPE_TRIATS(OVERLAP, TYPE) \
...
INSTANTIATE_RMSNORM_TYPE_TRIATS(0, OverlapType::prefetch)
INSTANTIATE_RMSNORM_TYPE_TRIATS(1, OverlapType::prefetch)
INSTANTIATE_RMSNORM_TYPE_TRIATS(2, OverlapType::prefetch)
INSTANTIATE_RMSNORM_TYPE_TRIATS(3, OverlapType::prefetch)
INSTANTIATE_RMSNORM_TYPE_TRIATS(4, OverlapType::prefetch)
INSTANTIATE_RMSNORM_TYPE_TRIATS(5, OverlapType::prefetch)
INSTANTIATE_RMSNORM_TYPE_TRIATS(6, OverlapType::prefetch)
INSTANTIATE_RMSNORM_TYPE_TRIATS(7, OverlapType::prefetch)
INSTANTIATE_RMSNORM_TYPE_TRIATS(8, OverlapType::prefetch)
INSTANTIATE_RMSNORM_TYPE_TRIATS(9, OverlapType::prefetch)
INSTANTIATE_RMSNORM_TYPE_TRIATS(10, OverlapType::prefetch)
...
INSTANTIATE_RMSNORM_TYPE_TRIATS(0, OverlapType::shared)
INSTANTIATE_RMSNORM_TYPE_TRIATS(1, OverlapType::shared)
INSTANTIATE_RMSNORM_TYPE_TRIATS(2, OverlapType::shared)
INSTANTIATE_RMSNORM_TYPE_TRIATS(3, OverlapType::shared)
INSTANTIATE_RMSNORM_TYPE_TRIATS(4, OverlapType::shared)
INSTANTIATE_RMSNORM_TYPE_TRIATS(5, OverlapType::shared)
INSTANTIATE_RMSNORM_TYPE_TRIATS(6, OverlapType::shared)
INSTANTIATE_RMSNORM_TYPE_TRIATS(7, OverlapType::shared)
INSTANTIATE_RMSNORM_TYPE_TRIATS(8, OverlapType::shared)
INSTANTIATE_RMSNORM_TYPE_TRIATS(9, OverlapType::shared)
INSTANTIATE_RMSNORM_TYPE_TRIATS(10, OverlapType::shared)
