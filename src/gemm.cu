...
#include "utils.cuh"
#include "helper.hpp"
void gemm_wrapper(int M, int N, int K,
...
void gemm_unpack(torch::Tensor A, torch::Tensor B, torch::Tensor C,
  ...
  gemm_wrapper(M, N, K, ptrA, ptrB, ptrC, ptrD, overlap_ratio,
                       prefetch_ratio);
}

void gemm_with_prefetch_type_check(torch::Tensor A, torch::Tensor B,
