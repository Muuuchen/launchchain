...
// 计算 RMSNorm 的 GFLOPS
void test_rmsnorm_gflops(int m, int n, float epsilon = 1e-5f,
                         int num_trials = 100) {
  ...
  // 预热
  rmsnorm_twoPassAlgo_e8_prefetch<5, 5, OverlapType::shared>
      <<<m, 256>>>(d_output, d_input, d_weight, m, n, epsilon);
  CUDA_CHECK(cudaGetLastError());
...
void test_rmsnorm_performance() {
  ...
  std::cout
      << "Testing templated version with OverlapScaled=5, PrefetchScaled=5"
      << std::endl;
  std::cout << "-----------------------------" << std::endl;
  ...
  for (int m : batch_sizes) {
    for (int n : feature_dims) {
      ...
      // 预热
      rmsnorm_twoPassAlgo_e8_prefetch<5, 5, OverlapType::prefetch>
          <<<m, 256>>>(d_output, d_input, d_weight, m, n, 1e-5f);
      cudaDeviceSynchronize();
...
int main() {
  test_rmsnorm_gflops(2048, 1024, 1e-5f, 1000);
  return 0;
}